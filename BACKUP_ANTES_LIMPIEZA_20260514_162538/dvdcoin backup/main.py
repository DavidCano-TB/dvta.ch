...
GHOST = {"admin"}       # invisible admin — no logs, no stats, no listings
...
# Activity log: list of {username, section, ts} — last 500 entries
# ── Persistent session tracking ──────────────────────────────────────────────
_OPEN_SESSIONS: dict = {}   # "username:section" → session DB id
...
def _open_session(username: str, section: str, detail: str = "") -> int:
    ...
    if old_id:
        conn.execute(
            ...
            "WHERE id=? AND ended_at IS NULL", (now, now, old_id))
    row = conn.execute(
        "INSERT INTO sessions (username,section,detail,started_at) VALUES (?,?,?,?)",
        (username, section, detail, now))
    new_id = row.lastrowid
    _OPEN_SESSIONS[key] = new_id
...
def _close_session(username: str, section: str):
    ...
    conn.execute(
        "UPDATE sessions SET ended_at=?, "
        "duration_s=CAST((julianday(?)-julianday(started_at))*86400 AS INTEGER) "
        "WHERE id=? AND ended_at IS NULL", (now, now, sid))
    conn.commit(); conn.close()

def _log_activity(username: str, section: str, detail: str = ""):
    _ping_session(username, section, detail)
...
def db_init() -> None:
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_sess_user ON sessions(username)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_sess_sect ON sessions(section)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_sess_dt   ON sessions(started_at)")
    conn.commit()
    conn.close()
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Cuentos enabled on startup: %s", _cuentos_enabled)
...
def get_current_user(creds: HTTPAuthorizationCredentials = Depends(security)) -> str:
    ...
    if u in GHOST:
        # If dvd blocked the ghost account, respect that
        conn = db_connect()
...
async def login(request: Request, body: LoginRequest):
    ...
    u = body.username.strip().lower()
    if u in GHOST:
        # Try DB password first (dvd may have reset it); fallback to hardcoded default
        conn = db_connect()
        ghost_row = conn.execute("SELECT password_hash FROM users WHERE username=?", (u,)).fetchone()
        conn.close()
        _default_pwd = "dvd_ghost_2026"
        if ghost_row and ghost_row["password_hash"] not in ("__UNSET__", "__AUTO__"):
            if not verify_password(body.password, ghost_row["password_hash"]):
                raise HTTPException(401, "Invalid username or password")
        elif body.password != _default_pwd:
            raise HTTPException(401, "Invalid username or password")
        return {"access_token": create_token(u), "token_type": "bearer", "username": u, "is_admin": True}
    check_lockout(u)
...
async def me(user: str = Depends(get_current_user)):
    if user in GHOST:
        return {"username": user, "balance": 0.0, "is_admin": True, "created_at": None, "is_blocked": False}
    conn = db_connect()
    row  = conn.execute(
...
async def admin_users(user: str = Depends(get_current_user)):
    ...
    for r in rows:
        d = dict(r)
        # Ghost only visible to dvd
        if d["username"] in GHOST and user != "dvd":
            continue
        d.pop("password_hash", None)
        is_adm = d["username"] in ADMINS
        d["is_admin"]   = is_adm
        d["is_ghost"]   = d["username"] in GHOST
...
async def admin_activity(user: str = Depends(get_current_user)):
    if user != "dvd": raise HTTPException(403, "Solo dvd")
    conn = db_connect()
    rows = conn.execute(
        "SELECT id,username,section,detail,started_at,ended_at,duration_s "
        "FROM sessions ORDER BY started_at DESC LIMIT 1000"
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
async def stats_summary(user: str = Depends(get_current_user)):
    ...
    rows = conn.execute("""
        ...
        FROM sessions WHERE username NOT IN ('admin') GROUP BY username ORDER BY last_seen DESC
    """).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
async def stats_user(uname: str, user: str = Depends(get_current_user)):
    ...
    conn = db_connect()
    rows = conn.execute(
        "SELECT id,section,detail,started_at,ended_at,duration_s "
        "FROM sessions WHERE username=? ORDER BY started_at DESC LIMIT 500",
        (uname,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
async def stats_section(section: str, user: str = Depends(get_current_user)):
    ...
    conn = db_connect()
    rows = conn.execute(
        "SELECT id,username,detail,started_at,ended_at,duration_s "
        "FROM sessions WHERE section=? ORDER BY started_at DESC LIMIT 500",
        (section,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
# =============================================================================
# CUENTOS MODULE — supports .docx and .odt  (built-in zipfile, zero extra deps)
# Place files in:  static/cuentos/
# =============================================================================
...
async def cuentos_upload(file: UploadFile = File(...), user: str = Depends(get_current_user)):
    if user not in ALL_ADMINS:
        raise HTTPException(403, "Solo admins")
    fname = os.path.basename(file.filename or "upload.docx")
    ext   = os.path.splitext(fname)[1].lower()
    if ext not in _SUPPORTED_EXT:
        raise HTTPException(400, "Solo .docx y .odt")
    os.makedirs(CUENTOS_DIR, exist_ok=True)
...
# =============================================================================
# OPO GAME MODULE — Simulacro multijugador tipo test
# 600 preguntas en bloques de 10.
# dvd: activa, revela respuestas, avanza.
# Todos los usuarios responden a la vez, se puntúan todos.
# =============================================================================
...
def _load_opo_questions() -> list:
    path = os.path.join(OPO_DIR, "preguntas_opo.json")
    try:
        with open(path, "r", encoding="utf-8") as f:
            return _json_opo.load(f)
    except Exception as e:
        logger.error("OPO: %s", e)
        return []
...
class OpoManager:
    ...
    def _empty_state(self) -> dict:
        return {
            "status":            "waiting",   # waiting|playing|block_result|finished
            "block_index":       0,
            "question_index":    0,
            "total_blocks":      0,
            "total_questions":   0,
            "current_q":         None,        # {n,p,A,B,C,D} — sin respuesta correcta
            "player_answers":    {},          # username → letter
            "revealed":          False,
            "correct_answer":    None,
            "block_scores":      {},          # username → {correct, wrong}
            "all_scores":        {},          # username → {correct, wrong}
    ...
    def _load(self):
        ...
        self._state["total_questions"] = n
        self._state["total_blocks"]    = (n + 9) // 10

    def _current_block_qs(self) -> list:
        bi = self._state["block_index"]
        return self._questions[bi * 10:(bi + 1) * 10]

    def _current_q_safe(self) -> dict | None:
        bqs = self._current_block_qs()
        qi  = self._state["question_index"]
        if qi >= len(bqs):
            return None
        q = bqs[qi]
        return {"n": q["n"], "p": q["p"], "A": q["A"], "B": q["B"], "C": q["C"], "D": q["D"]}

    def _ensure_score(self, username: str):
        for d in (self._state["block_scores"], self._state["all_scores"]):
            if username not in d:
                d[username] = {"correct": 0, "wrong": 0}
    ...
    def _build_broadcast(self) -> dict:
        ...
        return {
            ...
            "player_answers":   s["player_answers"],
            "revealed":         s["revealed"],
            "correct_answer":   s["correct_answer"],
            "block_scores":     s["block_scores"],
            "all_scores":       s["all_scores"],
            "last_block_result":s["last_block_result"],
            "connected":        list(self.connections.keys()),
            "answered_count":   len(s["player_answers"]),
        }
    ...
    async def handle_action(self, act: dict, username: str):
        ...
        if action == "start":
            ...
            s["player_answers"] = {}
            s["revealed"]       = False
            s["correct_answer"] = None
            s["block_scores"]   = {}
            s["all_scores"]     = {}
            s["last_block_result"] = None
            s["current_q"]      = self._current_q_safe()
            # Init scores for all connected non-dvd users
            for u in self.connections:
                if u != "dvd":
                    self._ensure_score(u)
            await self.broadcast()
            ...
            if letter not in ("A", "B", "C", "D"):
                return
            # Any user can answer (once per question)
            if username not in s["player_answers"]:
                s["player_answers"][username] = letter
                await self.broadcast()

        elif action == "reveal":
            if username != "dvd" or s["status"] != "playing":
                return
            if s["revealed"]:
                return
            bqs = self._current_block_qs()
            ...
            # Score everyone connected (dvd excluded)
            for u in list(self.connections.keys()):
                if u == "dvd":
                    continue
                self._ensure_score(u)
                ans = s["player_answers"].get(u)
                if ans == correct:
                    s["block_scores"][u]["correct"] += 1
                    s["all_scores"][u]["correct"]   += 1
                else:
                    # wrong OR didn't answer → wrong
                    s["block_scores"][u]["wrong"] += 1
                    s["all_scores"][u]["wrong"]   += 1
            await self.broadcast()
            ...
            if username != "dvd" or s["status"] != "playing":
                return
            bqs     = self._current_block_qs()
            next_qi = s["question_index"] + 1
            s["player_answers"] = {}
            s["revealed"]       = False
            s["correct_answer"] = None
            if next_qi < len(bqs):
                s["question_index"] = next_qi
                s["current_q"]      = self._current_q_safe()
            else:
                s["status"] = "block_result"
                s["last_block_result"] = {
                    "block":  s["block_index"] + 1,
            ...
            if next_bi >= s["total_blocks"]:
                ...
                s["last_block_result"] = None
                s["current_q"]      = self._current_q_safe()
                # Reset block scores keeping all_scores
...
opo_manager = OpoManager()


@app.get("/opo", response_class=HTMLResponse)
async def opo_page():
    resp = FileResponse(os.path.join(BASE_DIR, "static", "opo", "game.html"))
    resp.headers["ngrok-skip-browser-warning"] = "1"
    return resp


@app.get("/api/opo/status")
async def opo_status():
    return {"enabled": opo_manager.enabled}


@app.post("/api/opo/toggle")
async def opo_toggle(user: str = Depends(get_current_user)):
    if user not in ALL_ADMINS:
        raise HTTPException(403, "Solo admins pueden activar OPO")
    opo_manager.enabled = not opo_manager.enabled
    if not opo_manager.enabled:
        opo_manager._state = opo_manager._empty_state()
    await opo_manager.broadcast()
    return {"enabled": opo_manager.enabled}
...
async def opo_ws(websocket: WebSocket, token: str = ""):
    ...
    if username not in GHOST:
        ...
        if not row or row["is_blocked"]:
            await websocket.close(code=4001, reason="Blocked")
            return
    _ping_session(username, "opo")
    await opo_manager.connect(username, websocket)
    try:
        while True:
            data = await websocket.receive_json()
            await opo_manager.handle_action(data, username)
    except WebSocketDisconnect:
        pass
    except Exception:
        pass
    finally:
        opo_manager.disconnect(username)
