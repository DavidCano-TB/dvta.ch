...
def db_init():
    ...
    # Add contact fields for notifications (migration safety)
    try:
    ...
    try:
        ...
        pass
    try:
        c.execute("ALTER TABLE users ADD COLUMN notifications_email INTEGER NOT NULL DEFAULT 1")
        c.commit()
    except Exception:
        pass
    try:
        c.execute("ALTER TABLE users ADD COLUMN notifications_sms INTEGER NOT NULL DEFAULT 0")
        c.commit()
    except Exception:
        pass
    try:
        c.execute("ALTER TABLE users ADD COLUMN notifications_whatsapp INTEGER NOT NULL DEFAULT 0")
        c.commit()
    except Exception:
        pass
    c.close()
...
@app.post("/bank/api/me/update-profile")
async def update_profile(body: UpdateProfileRequest, user: str = Depends(get_current_user)):
    ...
    if body.whatsapp is not None:
        ...
        params.append(body.whatsapp.strip() if body.whatsapp else None)
    
    if body.notifications_email is not None:
        updates.append("notifications_email = ?")
        params.append(1 if body.notifications_email else 0)
    
    if body.notifications_sms is not None:
        updates.append("notifications_sms = ?")
        params.append(1 if body.notifications_sms else 0)
    
    if body.notifications_whatsapp is not None:
        updates.append("notifications_whatsapp = ?")
        params.append(1 if body.notifications_whatsapp else 0)
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info("Profile updated for @%s", user)
        return {"ok": True, "message": "Profile updated successfully"}
    except Exception as e:
        conn.close()
        logger.error("Error updating profile for @%s: %s", user, str(e))
...
async def me(user: str = Depends(get_current_user)):
    ...
    try:
        ...
        if "notifications_email" in keys:
            notifications_email = bool(row["notifications_email"])
        if "notifications_sms" in keys:
            notifications_sms = bool(row["notifications_sms"])
        if "notifications_whatsapp" in keys:
            notifications_whatsapp = bool(row["notifications_whatsapp"])
    except Exception:
        pass
...
class QuienSoyManager:
    ...
    def _empty_state(self) -> dict:
        return {
            ...
            "character_info": None,      # NUEVO: información completa del personaje (características, etc.)
    ...
    def _build_broadcast(self, viewer: str = None) -> dict:
        ...
        return {
            ...
            "character_photo": s.get("character_photo") if is_host else None,
            "players":    players_pub,
    ...
    async def _ask_ai(self, character_info: dict, question: str) -> str:
        """IA 100% - Usa SOLO Gemini API con información completa del personaje."""
        try:
            character_name = character_info.get("corrected_name", character_info.get("character", "Unknown"))
            logger.info(f"QuienSoy AI: Pregunta='{question}' sobre Personaje='{character_name}'")
            logger.info(f"QuienSoy AI: Características={character_info.get('key_characteristics', [])}")
            
            # Usar SOLO Gemini API con información completa
            from ai_helper import ask_quien_soy
            response = ask_quien_soy(character_info, question)
            
            logger.info(f"QuienSoy AI: Respuesta de Gemini = '{response}'")
            
            # SOLO SE ACEPTAN "Sí" o "No" - NO "Ni sí ni no"
            if response in ["Sí", "No"]:
                logger.info(f"QuienSoy AI (Gemini): Respuesta válida = {response}")
                return response
            else:
                logger.warning(f"QuienSoy AI: Respuesta inválida de Gemini: '{response}', usando 'No'")
                return "No"
                
        except Exception as e:
            logger.error(f"QuienSoy AI: Error con Gemini API: {e}", exc_info=True)
            return "No"
    ...
    async def handle_action(self, act: dict, admin: str):
        ...
        if action == "setup":
            ...
            # Usar información completa del personaje
            character_info = s.get("character_info")
            if not character_info:
                # Fallback: crear info básica
                character_info = {
                    "corrected_name": s["character"],
            ...
            answer = await self._ask_ai(character_info, question)
            ...
            # Comparación flexible: normalizar ambos nombres
            ...
            character_normalized = normalize_name(s["character"])
            
            # Comparación exacta normalizada
            correct = guess_normalized == character_normalized
            
            # Si no es exacto, intentar comparación fonética/flexible
            if not correct:
                # Permitir coincidencias parciales si el guess contiene el personaje o viceversa
                if len(guess_normalized) >= 3 and len(character_normalized) >= 3:
                    # Si uno contiene al otro (para casos como "mario" vs "mario bros")
                    if guess_normalized in character_normalized or character_normalized in guess_normalized:
                        # Solo aceptar si la diferencia no es muy grande
                        len_diff = abs(len(guess_normalized) - len(character_normalized))
                        if len_diff <= 5:  # Máximo 5 caracteres de diferencia
                            correct = True
...
async def quien_soy_verify(name: str, user: str = Depends(get_current_user)):
    ...
    ULTRA FLEXIBLE: Acepta casi cualquier personaje conocido.
    """
    if user not in ADMINS:
        raise HTTPException(403, "Admins only")
    name = name.strip()
    ...
    logger.info(f"verify-character: Verificando '{name}' con IA (100%% IA, ultra flexible)")
    
    # Lista de sugerencias por defecto
    default_suggestions = [
        {"name": "Scooby-Doo", "photo": None},
        {"name": "Mickey Mouse", "photo": None},
        {"name": "Superman", "photo": None},
        {"name": "Batman", "photo": None},
        {"name": "Spider-Man", "photo": None},
        {"name": "Harry Potter", "photo": None},
        {"name": "Pikachu", "photo": None},
        {"name": "Mario Bros", "photo": None},
        {"name": "Sonic", "photo": None},
        {"name": "Bugs Bunny", "photo": None}
    ]
    ...
    try:
        ...
        logger.info("Using Gemini AI for character verification (ULTRA FLEXIBLE)")
        char_info = ai.verify_character(name)
        
        logger.info(f"AI verification result: exists={char_info.get('exists')}, confidence={char_info.get('confidence')}, corrected={char_info.get('corrected_name')}")
        
        # Obtener sugerencias de la IA o usar las por defecto
        ai_suggestions = char_info.get("suggestions", [])
        if not ai_suggestions or ai_suggestions == [None] or any(s is None or s == "undefined" for s in ai_suggestions):
            logger.warning(f"AI suggestions invalid: {ai_suggestions}, using defaults")
            ai_suggestions = [s["name"] for s in default_suggestions]
        
        # Formatear sugerencias
        suggestions_formatted = []
        for suggestion in ai_suggestions[:10]:
            if suggestion and suggestion != "undefined" and suggestion != "None":
        ...
        if not suggestions_formatted:
            suggestions_formatted = default_suggestions
        
        # ULTRA FLEXIBLE: Aceptar si existe=True O si tiene confianza media/alta
        should_accept = (
            char_info.get("exists", False) or 
            char_info.get("confidence") in ["high", "medium"]
        )
        
        if should_accept:
            corrected = char_info.get("corrected_name", name.title())
            # Validar que corrected_name no sea None, undefined, null o vacío
            if not corrected or str(corrected).lower() in ["none", "undefined", "null", ""]:
                corrected = name.title()
            
            # Asegurar que corrected es un string válido
            corrected = str(corrected).strip()
            if not corrected:
                corrected = name.title()
            
            logger.info(f"✓ AI ACEPTÓ: '{corrected}' (confidence: {char_info.get('confidence')})")
        ...
        # Si la IA rechaza (muy raro con el nuevo prompt), ofrecer sugerencias
        else:
            logger.warning(f"⚠ AI rechazó: '{name}' (confidence: {char_info.get('confidence')})")
            ...
            return {
                ...
                "photo": None,
                "suggestions": suggestions_formatted,
                "message": "Personaje no reconocido. Elige uno de los personajes sugeridos:",
                "ai_info": {
        ...
        # FALLBACK: En caso de error, ofrecer sugerencias populares
        return {
            "valid": False,
            "canonical": name.title(),
            "photo": None,
            "suggestions": default_suggestions,
            "message": f"Error al verificar personaje. Elige uno de los sugeridos. Error: {str(e)}",
            "ai_info": {
                "reason": "Error en servicio de IA",
                "confidence": "low"
            }
        }
...
class OpoManager:
    def __init__(self):
        self.enabled:     bool = False
        self.connections: dict = {}        # username → websocket
        self._questions:  list = _load_opo_questions()  # Load questions on init
        self._state:      dict = self._empty_state()
        self._auto_task         = None
        # Set total_blocks based on loaded questions
        if self._questions:
