...
tasklist /fi "IMAGENAME eq python.exe" /fo list 2>nul | findstr /i "PID" > temp_watchdog_pids_scripts.txt
for /f "tokens=2" %%a in (temp_watchdog_pids_scripts.txt) do (
...
del temp_watchdog_pids_scripts.txt 2>nul
