...
tasklist /fi "IMAGENAME eq python.exe" /fo list 2>nul | findstr /i "PID" > temp_stop_watchdog.txt
for /f "tokens=2" %%a in (temp_stop_watchdog.txt) do (
...
del temp_stop_watchdog.txt 2>nul
